...
/// <summary>
/// Generates, inside Wino.AppServices.Contracts, for every [WinoRpcService] interface found in the
/// referenced Wino assemblies:
///  - strongly typed request (and tuple response) records per remoted method,
///  - a UI side {Name}RemoteProxy implementing the interface over IWinoRpcClient,
...
/// Members whose signatures cannot cross the AppService boundary produce compile errors unless excluded.
/// </summary>
[Generator]
public sealed class RpcGenerator : IIncrementalGenerator
{
    ...
    [
        "Get", "Fetch", "Search", "Find", "Is", "Are", "Has", "Should", "Check", "Test", "Download", "Ping",
        "Expand", "Populate"
    ];
    ...
    private sealed class MethodModel
    {
        ...
        public string RecordBaseName { get; set; } = string.Empty;
        public bool IsExcluded { get; set; }
        public bool IsAsync { get; set; }
        public bool HasReturnValue { get; set; }
        public string ReturnTypeDisplay { get; set; } = string.Empty; // inner T (without Task<>)
        public string DeclaredReturnTypeDisplay { get; set; } = string.Empty;
    ...
    private sealed class InterfaceModel
    {
        ...
        public string InterfaceDisplay { get; set; } = string.Empty;
        public string ProxyName { get; set; } = string.Empty;
        public string HybridProxyName { get; set; } = string.Empty;
        public string DispatcherName { get; set; } = string.Empty;
        public string ServiceFieldName { get; set; } = string.Empty;
    ...
    private static InterfaceModel? BuildInterfaceModel(SourceProductionContext context, INamedTypeSymbol serviceInterface)
    {
        ...
        var nameWithoutPrefix = serviceInterface.Name.StartsWith("I", StringComparison.Ordinal) ? serviceInterface.Name.Substring(1) : serviceInterface.Name;
        ...
        return new InterfaceModel
        {
            ...
            HybridProxyName = $"{nameWithoutPrefix}HybridProxy",
                                                 ...
                                                 ref bool hasErrors)
    {
        ...
        var isWrite = !ReadMethodPrefixes.Any(prefix => method.Name.StartsWith(prefix, StringComparison.Ordinal)) &&
                      !method.Name.EndsWith("ExistsAsync", StringComparison.Ordinal);

        return new MethodModel
        {
            Name = method.Name,
            Uid = BuildStableMethodId(serviceInterface, method),
    ...
    private static string BuildStableMethodId(INamedTypeSymbol serviceInterface, IMethodSymbol method)
    {
        ...
        return $"{serviceInterface.Name}.{method.Name}({parameterTypes})#v1";
    }
    ...
    private static string GenerateInterfaceSource(InterfaceModel model)
    {
        ...
        AppendHybridProxy(builder, model);
    ...
    {
        builder.AppendLine($"    /// <summary>UI-side proxy for <see cref=\"{model.InterfaceDisplay}\"/> over AppService.</summary>");
        builder.AppendLine($"    public sealed class {model.ProxyName} : {model.InterfaceDisplay}");
        builder.AppendLine("    {");
        builder.AppendLine("        private readonly global::Wino.AppServices.Contracts.IWinoRpcClient _rpcClient;");
        builder.AppendLine();
        builder.AppendLine($"        public {model.ProxyName}(global::Wino.AppServices.Contracts.IWinoRpcClient rpcClient)");
        builder.AppendLine("        {");
        ...
        foreach (var method in model.Methods)
        {
            ...
            if (method.HasReturnValue)
            {
                invocation = $"_rpcClient.InvokeAsync(\"{method.Uid}\", {requestCreation}, " +
                             $"global::Wino.AppServices.Contracts.WinoRpcJson.GetTypeInfo<{method.RecordBaseName}Request>(), " +
                             $"global::Wino.AppServices.Contracts.WinoRpcJson.GetTypeInfo<{responseTypeDisplay}>(), " +
    ...
    private static void AppendHybridProxy(StringBuilder builder, InterfaceModel model)
    {
        builder.AppendLine($"    /// <summary>UI-side split proxy: database reads stay local; mutations use AppService.</summary>");
        builder.AppendLine($"    public sealed class {model.HybridProxyName} : {model.InterfaceDisplay}");
        builder.AppendLine("    {");
        builder.AppendLine($"        private readonly {model.InterfaceDisplay} _local;");
        builder.AppendLine($"        private readonly {model.ProxyName} _remote;");
        builder.AppendLine();
        builder.AppendLine($"        public {model.HybridProxyName}({model.InterfaceDisplay} local, {model.ProxyName} remote)");
        builder.AppendLine("        {");
        builder.AppendLine("            _local = local;");
        ...
        foreach (var method in model.Methods)
        {
            ...
            var target = method.IsExcluded || !method.IsWrite ? "_local" : "_remote";
            builder.AppendLine($"        public {method.DeclaredReturnTypeDisplay} {method.Name}({parameterList})");
            builder.AppendLine($"            => {target}.{method.Name}({arguments});");
        }

        foreach (var property in model.ExcludedProperties)
        {
            builder.AppendLine();
            builder.AppendLine($"        public {property.TypeDisplay} {property.Name}");
            builder.AppendLine("        {");
            if (property.HasGetter)
                builder.AppendLine($"            get => _local.{property.Name};");
            if (property.HasSetter)
                builder.AppendLine($"            set => _local.{property.Name} = value;");
            builder.AppendLine("        }");
        }
        ...
        foreach (var eventModel in model.ExcludedEvents)
        {
            ...
            builder.AppendLine($"        public event {eventModel.TypeDisplay} {eventModel.Name}");
            builder.AppendLine("        {");
            builder.AppendLine($"            add => _local.{eventModel.Name} += value;");
            builder.AppendLine($"            remove => _local.{eventModel.Name} -= value;");
            builder.AppendLine("        }");
        }
    ...
    private static void AppendDispatcher(StringBuilder builder, InterfaceModel model)
    {
        ...
        builder.AppendLine($"    public sealed class {model.DispatcherName} : global::Wino.AppServices.Contracts.IWinoRpcRequestHandler");
        builder.AppendLine("    {");
        ...
        foreach (var method in model.Methods.Where(m => !m.IsExcluded))
        {
            ...
            if (hasPayloadParameters)
                builder.AppendLine($"                    var request = global::Wino.AppServices.Contracts.WinoRpcJson.Deserialize<{method.RecordBaseName}Request>(payload);");
            ...
            else if (method.TupleElements != null)
            {
                ...
                builder.AppendLine($"                    return global::Wino.AppServices.Contracts.WinoRpcJson.SerializeToUtf8Bytes(new {method.RecordBaseName}Response({responseArguments}));");
            }
            else
            {
                builder.AppendLine($"                    var result = {serviceCall};");
                builder.AppendLine("                    return global::Wino.AppServices.Contracts.WinoRpcJson.SerializeToUtf8Bytes(result);");
            }
    ...
    private static string GenerateComposedDispatcher(List<InterfaceModel> models)
    {
        ...
        builder.AppendLine("    public sealed class WinoRpcDispatcher : global::Wino.AppServices.Contracts.IWinoRpcRequestHandler");
        builder.AppendLine("    {");
        builder.AppendLine("        private readonly global::System.Collections.Generic.Dictionary<string, global::Wino.AppServices.Contracts.IWinoRpcRequestHandler> _dispatchers = new();");
        builder.AppendLine();
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public static WinoRpcDispatcher FromServices(global::System.IServiceProvider services)");
        builder.AppendLine("        {");
        builder.AppendLine("            if (services is null) throw new global::System.ArgumentNullException(nameof(services));");
        builder.AppendLine("            return new WinoRpcDispatcher(");

        for (var index = 0; index < models.Count; index++)
        {
            var model = models[index];
            var suffix = index == models.Count - 1 ? ");" : ",";
            builder.AppendLine($"                ({model.InterfaceDisplay})(services.GetService(typeof({model.InterfaceDisplay})) ?? throw new global::System.InvalidOperationException(\"Missing companion service {model.InterfaceName}.\")){suffix}");
        }
        ...
        {
            var nameWithoutPrefix = model.InterfaceName.StartsWith("I", StringComparison.Ordinal) ? model.InterfaceName.Substring(1) : model.InterfaceName;
