...
public class MailService : BaseDatabaseService, IMailService
{
    ...
    private MimeMessage CreateEmptyDraft(BodyBuilder builder, MimeMessage message, DraftCreationOptions draftCreationOptions)
    {
        ...
        {
            if (draftCreationOptions.MailToParameters.Subject != null)
                message.Subject = draftCreationOptions.MailToParameters.Subject;

            if (draftCreationOptions.MailToParameters.Body != null)
            {
                builder.TextBody = draftCreationOptions.MailToParameters.Body;
                builder.HtmlBody = draftCreationOptions.MailToParameters.Body;

                message.Body = builder.ToMessageBody();
            }

            if (draftCreationOptions.MailToParameters.To != null)
                message.To.Add(new MailboxAddress(draftCreationOptions.MailToParameters.To, draftCreationOptions.MailToParameters.To));

            if (draftCreationOptions.MailToParameters.Cc != null)
                message.Cc.Add(new MailboxAddress(draftCreationOptions.MailToParameters.Cc, draftCreationOptions.MailToParameters.Cc));

            if (draftCreationOptions.MailToParameters.Bcc != null)
                message.Bcc.Add(new MailboxAddress(draftCreationOptions.MailToParameters.Bcc, draftCreationOptions.MailToParameters.Bcc));
        }
