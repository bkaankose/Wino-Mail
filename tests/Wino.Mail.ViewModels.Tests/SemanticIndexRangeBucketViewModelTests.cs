...
public sealed class SemanticIndexRangeBucketViewModelTests
{
    ...
    {
        var bucket = Create(localAndCloud: 4, localOnly: 3, cloudOnly: 1, height: 64);

        bucket.LocalAndCloudHeight.Should().Be(32);
        bucket.LocalOnlyHeight.Should().Be(24);
        bucket.CloudOnlyHeight.Should().Be(8);
        (bucket.LocalAndCloudHeight + bucket.LocalOnlyHeight + bucket.CloudOnlyHeight).Should().Be(64);
        bucket.EmptyHeight.Should().Be(0);
    }

    [Fact]
    public void EmptyCoverage_UsesNeutralFullHeightSegment()
    {
        var bucket = Create(localAndCloud: 0, localOnly: 0, cloudOnly: 0, height: 2);
    ...
    private static SemanticIndexRangeBucketViewModel Create(int localAndCloud, int localOnly, int cloudOnly, double height)
        => new()
        {
            StartOffset = 0,
            EndOffset = 0,
            ...
            MessageCount = localAndCloud + localOnly + cloudOnly,
