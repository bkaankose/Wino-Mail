    ...
    IRecipient<WinoAccountProfileDeletedMessage>
{
    ...
    private IMailDialogService MailDialogService => App.Current.Services.GetRequiredService<IMailDialogService>();
    private IWinoAccountProfileService WinoAccountProfileService => App.Current.Services.GetRequiredService<IWinoAccountProfileService>();

    public WinoAppShell()
    {
        InitializeComponent();
        App.Current.WindowPresentationManager.RegisterTitleBar(ShellTitleBar);
        ...
        RefreshTitleBarBackButtonVisibility();
    }
    ...
    {
        var mode = _activeMode ?? ViewModel.StatePersistenceService.ApplicationMode;
        DeactivateCurrentMode();
        _activeMode = null;
        ActivateMode(mode, new ShellModeActivationContext
        {
    ...
    private void TitleBarBackButtonClicked(object? sender, RoutedEventArgs e)
    {
        ViewModel.NavigationService.GoBack();
        RefreshTitleBarBackButtonVisibility();
    }

    private void TitleBarPaneButtonClicked(object? sender, RoutedEventArgs e)
    {
        var isOpen = !ViewModel.PreferencesService.IsNavigationPaneOpened;
        ViewModel.PreferencesService.IsNavigationPaneOpened = isOpen;
    ...
    public void Receive(TitleBarShellContentUpdated message)
    {
        ...
        {
            DispatcherQueue.TryEnqueue(() => Receive(message));
            return;
        }
    ...
    {
        _activeTitleBarSearchHost = InnerShellFrame.Content as ITitleBarSearchHost;
    ...
    private void RefreshTitleBarBackButtonVisibility()
    {
        ShellTitleBar.IsBackButtonVisible = ViewModel.NavigationService.CanGoBack();
    }
    ...
    private async void TitleBarSearchQuerySubmitted(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
    {
        ...
        await _activeTitleBarSearchHost.OnTitleBarSearchSubmittedAsync(
            args.QueryText,
            args.ChosenSuggestion as TitleBarSearchSuggestion);
        SynchronizeTitleBarSearchBox();
    }

    private void ShowInfoBarMessage(InfoBarMessageRequested message)
    {
        ShellInfoBar.ActionButton = string.IsNullOrEmpty(message.ActionButtonTitle) || message.Action == null
            ? null
            : new Button
            {
    ...
    {
        if (e.PropertyName == nameof(ViewModel.CurrentMenuItems))
        {
            RefreshNavigationViewBindings(syncMailSelection: false);
            return;
        }
    ...
    private void StatePersistenceServiceChanged(object? sender, string propertyName)
    {
        if (!DispatcherQueue.HasThreadAccess)
        {
            if (!DispatcherQueue.TryEnqueue(() => StatePersistenceServiceChanged(sender, propertyName)))
    ...
    private static ModifierKeys GetCurrentModifierKeys()
    {
        ...
        if (Windows.UI.Core.CoreWindow.GetForCurrentThread().GetKeyState(Windows.System.VirtualKey.Control).HasFlag(Windows.UI.Core.CoreVirtualKeyStates.Down))
            modifiers |= ModifierKeys.Control;
        if (Windows.UI.Core.CoreWindow.GetForCurrentThread().GetKeyState(Windows.System.VirtualKey.Menu).HasFlag(Windows.UI.Core.CoreVirtualKeyStates.Down))
            modifiers |= ModifierKeys.Alt;
        if (Windows.UI.Core.CoreWindow.GetForCurrentThread().GetKeyState(Windows.System.VirtualKey.Shift).HasFlag(Windows.UI.Core.CoreVirtualKeyStates.Down))
            modifiers |= ModifierKeys.Shift;
        if (Windows.UI.Core.CoreWindow.GetForCurrentThread().GetKeyState(Windows.System.VirtualKey.LeftWindows).HasFlag(Windows.UI.Core.CoreVirtualKeyStates.Down) ||
            Windows.UI.Core.CoreWindow.GetForCurrentThread().GetKeyState(Windows.System.VirtualKey.RightWindows).HasFlag(Windows.UI.Core.CoreVirtualKeyStates.Down))
        {
