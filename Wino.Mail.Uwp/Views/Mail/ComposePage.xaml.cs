    ...
    IRecipient<ApplicationThemeChanged>
{
    ...
    private async void OnImageDropGridImageDropped(object sender, DragEventArgs e)
    {
        try
        {
            if (e.DataView.Contains(StandardDataFormats.StorageItems))
            {
                ...
                await WebViewEditor.InsertImagesAsync(imagesInformation.Select(image =>
                    new Wino.Mail.Editor.EditorImageInfo(image.Data, image.Name)));
            }
        }
        // State should be reset even when an exception occurs, otherwise the UI will be stuck in a dragging state.
        finally
    ...
    public Task FlushDraftAsync() => ViewModel.UpdateMimeChangesAsync();
