...
public sealed class WinoAppShellViewModel : CoreBaseViewModel, IShellViewModel
{
    ...
    {
        if (StatePersistenceService.ApplicationMode != mode)
        {
            StatePersistenceService.ApplicationMode = mode;
        }

        StatePersistenceService.AppModeTitle = mode switch
        {
            ...
            // UWP appends the package display name (Wino Mail) to a non-empty
            // ApplicationView title. Keep Mail empty to avoid "Wino Mail - Wino Mail".
            _ => string.Empty,
        };
