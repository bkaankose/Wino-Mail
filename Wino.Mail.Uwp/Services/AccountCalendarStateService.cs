    ...
    IRecipient<AccountSynchronizationProgressUpdatedMessage>
{
    ...
    public void AddGroupedAccountCalendar(GroupedAccountCalendarViewModel groupedAccountCalendar)
    {
        lock (_calendarStateLock)
        {
            ...
            groupedAccountCalendar.ApplySynchronizationProgress(
                Wino.Core.Domain.Models.Synchronization.AccountSynchronizationProgress.Idle(
                    groupedAccountCalendar.Account.Id,
                    SynchronizationProgressCategory.Calendar));
            _ = RefreshInitialSynchronizationProgressAsync(groupedAccountCalendar);
    ...
    private async Task RefreshInitialSynchronizationProgressAsync(
        GroupedAccountCalendarViewModel groupedAccountCalendar)
    {
        try
        {
            var progress = await Task.Run(() => _synchronizationManager.GetSynchronizationProgress(
                    groupedAccountCalendar.Account.Id,
                    SynchronizationProgressCategory.Calendar))
            ...
            if (dispatcher is not null)
            {
                ...
                {
                    groupedAccountCalendar.ApplySynchronizationProgress(progress);
                    UpdateAggregateSynchronizationState();
                });
            }
        }
        ...
        {
            // Live synchronization events will update the idle initial state once
            // the companion is attached. Calendar startup remains independent.
        }
    }
