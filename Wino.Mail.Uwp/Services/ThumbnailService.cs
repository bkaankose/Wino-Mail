    ...
    IApplicationConfiguration applicationConfiguration) : IThumbnailService
{
    ...
    private async ValueTask<ThumbnailCacheEntry> GetThumbnailInternal(string email, bool awaitLoad)
    {
        ...
        var existingEntry = GetExistingThumbnailEntry(email);
    ...
    private ThumbnailCacheEntry? GetExistingThumbnailEntry(string email)
    {
        if (!Directory.Exists(_thumbnailCacheFolderPath))
            return null;

        var emailHash = ComputeEmailHash(email);
        var gravatarFileName = FindExistingVariant($"{emailHash}-gravatar");
        var faviconFileName = FindExistingVariant($"{emailHash}-favicon");
    ...
    private string? FindExistingVariant(string filePrefix)
        => Directory.EnumerateFiles(_thumbnailCacheFolderPath, $"{filePrefix}.*")
            .Select(Path.GetFileName)
            .FirstOrDefault(static fileName => !string.IsNullOrWhiteSpace(fileName));
