...
- For dynamic menus, define `Wino.Mail.Controls.Core.ContextFlyout` entries and pass them through `ItemsSource` and `HeaderItemsSource`. For fixed or bindable menus, declare `WinoContextFlyoutItem`, toggle/radio items, separators, and nested submenu items directly inside `WinoContextFlyout` in XAML.
