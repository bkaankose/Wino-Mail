...
public sealed partial class WinoMailEditor : UserControl, IEditorCommandTarget, IDisposable
{
    ...
    public static readonly DependencyProperty WebViewModeProperty = DependencyProperty.Register(
        nameof(WebViewMode), typeof(EditorWebViewMode), typeof(WinoMailEditor),
    ...
    public static readonly DependencyProperty LoadWebView2Property = DependencyProperty.Register(
        nameof(LoadWebView2), typeof(bool), typeof(WinoMailEditor), new PropertyMetadata(false));
    public static readonly DependencyProperty LoadCompatibilityWebViewProperty = DependencyProperty.Register(
        nameof(LoadCompatibilityWebView), typeof(bool), typeof(WinoMailEditor), new PropertyMetadata(false));
    ...
    public EditorWebViewMode WebViewMode { get => (EditorWebViewMode)GetValue(WebViewModeProperty); set => SetValue(WebViewModeProperty, value); }
    public bool LoadWebView2 { get => (bool)GetValue(LoadWebView2Property); private set => SetValue(LoadWebView2Property, value); }
    public bool LoadCompatibilityWebView { get => (bool)GetValue(LoadCompatibilityWebViewProperty); private set => SetValue(LoadCompatibilityWebViewProperty, value); }
    public EditorWebViewMode ActiveWebViewMode => _bridge?.ActiveMode ?? EditorWebViewMode.Auto;
    public bool IsEditorWebViewEditor => true;
    ...
    public async Task<string?> GetHtmlBodyAsync() => await GetHtmlAsync();
    ...
    public Task FocusEditorAsync(bool _) => FocusEditorAsync();

    public Microsoft.UI.Xaml.Controls.WebView2 GetUnderlyingWebView() =>
        ActiveWebViewMode == EditorWebViewMode.WebView2 && EditorWebView2 is not null
            ? EditorWebView2
            : throw new InvalidOperationException("The editor is not currently using WebView2.");

    public WebView GetUnderlyingCompatibilityWebView() =>
        ActiveWebViewMode == EditorWebViewMode.WebView && CompatibilityWebView is not null
            ? CompatibilityWebView
            : throw new InvalidOperationException("The editor is not currently using the compatibility WebView.");
    ...
    private async void WinoMailEditor_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        if (_bridge is null || _bridge.ActiveMode != EditorWebViewModeResolver.Resolve(WebViewMode))
    ...
    private async Task SwitchWebViewModeAsync(EditorWebViewMode requestedMode)
    {
        ...
        if (_bridge?.ActiveMode == resolvedMode) return;

        string? currentHtml = await TryGetCurrentHtmlAsync();
        DisposeBridge();
        UnloadBrowserHosts();

        try
        {
            await LoadAndInitializeBridgeAsync(resolvedMode);
        }
            ...
            !_disposed)
        {
            DisposeBridge();
            UnloadBrowserHosts();
            await LoadAndInitializeBridgeAsync(EditorWebViewMode.WebView);
        }

        if (currentHtml is not null)
            await _bridge!.SetContentAsync(currentHtml);
        ...
        SetStatus($"Ready ({_bridge.ActiveMode})", false);
    }

    private async Task LoadAndInitializeBridgeAsync(EditorWebViewMode mode)
    {
        ...
        {
            LoadCompatibilityWebView = true;
        }

        FrameworkElement browser = await _browserLoadedSource.Task;
        ObjectDisposedException.ThrowIf(_disposed, this);

        _bridge = browser switch
        {
            Microsoft.UI.Xaml.Controls.WebView2 modernWebView => new EditorBridge(modernWebView),
            WebView compatibilityWebView => new EditorBridge(compatibilityWebView),
            _ => throw new InvalidOperationException("The requested editor browser could not be created.")
        };
    ...
    private async Task<string?> TryGetCurrentHtmlAsync()
    {
        if (_bridge is null) return null;
        try { return await _bridge.GetContentAsync(); }
        catch { return null; }
    }
    ...
    private void EditorWebView2_Loaded(object sender, RoutedEventArgs e) =>
        _browserLoadedSource?.TrySetResult((FrameworkElement)sender);

    private void CompatibilityWebView_Loaded(object sender, RoutedEventArgs e) =>
        _browserLoadedSource?.TrySetResult((FrameworkElement)sender);

    private async void CompatibilityWebView_NavigationCompleted(WebView sender, WebViewNavigationCompletedEventArgs args)
    {
        if (!args.IsSuccess)
        {
            SetStatus($"Compatibility WebView navigation failed: {args.WebErrorStatus}", true);
            return;
        }

        if (_bridge is not null)
        {
            await _bridge.ValidateLegacyNavigationAsync();
        }
    }

    private void CompatibilityWebView_ScriptNotify(object sender, NotifyEventArgs e) =>
        _bridge?.ReceiveLegacyMessage(e.Value);
    ...
    public async Task ExecuteCommandAsync(EditorCommand command)
    {
        ...
        switch (command.Kind)
        {
            ...
            case EditorCommandKind.ToggleBuiltInToolbar: ToolbarVisibility = command.Value is true ? Visibility.Visible : Visibility.Collapsed; break;
    ...
    private static void OnWebViewModeChanged(DependencyObject sender, DependencyPropertyChangedEventArgs args)
    {
        var editor = (WinoMailEditor)sender;
        if (!editor._disposed && editor._isLoaded)
        {
            editor.ObserveWebViewModeSwitch(editor.QueueWebViewModeSwitch());
        }
    }

    private async void ObserveWebViewModeSwitch(Task switchTask)
    {
        try { await switchTask; }
        catch (Exception exception) when (ReferenceEquals(switchTask, _initializationTask))
        {
            SetStatus($"Editor failed to switch browser: {exception.Message}", true);
        }
        catch
        {
            // A newer mode switch owns the editor state and reports its result.
        }
    }
    ...
    public void Dispose()
    {
        ...
        _browserLoadedSource?.TrySetException(new ObjectDisposedException(nameof(WinoMailEditor)));
        DisposeBridge();
        UnloadBrowserHosts();
