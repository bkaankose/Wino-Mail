...
/// <summary>
/// Interactive OAuth2 sign-in. The default implementation uses the system browser and a loopback
/// redirect; the WinUI head replaces it with an embedded WebView2 host that captures the redirect
/// in-process, which is what an https reply URL (the OWA convention) needs.
/// </summary>
...
public sealed class LoopbackRedirectListener : IDisposable
{
    ...
    private static async Task WriteCompletionPageAsync(NetworkStream stream, CancellationToken cancellationToken)
    {
        const string html = "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><title>Sign-in complete</title></head>" +
            ...
            "<h2>Sign-in complete</h2><p>You can close this window and return to Wino Mail.</p></body></html>";
